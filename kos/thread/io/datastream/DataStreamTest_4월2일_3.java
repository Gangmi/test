...
public class DataStreamTest_4월2일_3 {
		...
		public void actionPerformed(ActionEvent ev) {
			...
			if (evtBtn == bSave) {
				...
				try {
					...
					// 데이터를 다 살리고 싶어서 필터를 씌움 만들어 놓은 통로를 아래에 필터링을 담당하는 코드를 씌워줌
					// 이렇게 필터를 달았으면, 필터를 통해서 output 을 해야 한다.
					...
					//이거 하나 쓰겠다고 4번을 코딩하는것은 너무 비효율적임
					//이걸 묶어서 단 한번만 라이팅 하고싶을 때
					dos.close();


					//언제까지 있을지 모르니까 while 
//					while (true) {
//						int data = dis.read(); // dis의 통로를 이용해서 데이터를 읽어온다.
//						if (data == -1) 
//							break;
//
//						dis.read
