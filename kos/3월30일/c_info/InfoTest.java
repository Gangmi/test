...
public class InfoTest extends JFrame {
	...
	public InfoTest() {
		...
		badd = new JButton("add" , new ImageIcon("3월30일/c_info/imgs/2.PNG")); // 버튼에 이미지 넣기
		badd.setVerticalTextPosition(JButton.BOTTOM); // 수직정렬
		badd.setHorizontalTextPosition(JButton.CENTER); // 수평정렬
		badd.setPressedIcon(new ImageIcon("3월30일/c_info/imgs/3.PNG")); //눌렸을 때 바뀌는 아이콘
		badd.setRolloverIcon(new ImageIcon("3월30일/c_info/imgs/4.PNG")); // 갔다 댔을 때 바뀌는 아이콘
		badd.setToolTipText("사용자 정보를 입력 하세요");
		badd.setMnemonic('i'); // 알트와 함께 눌러야 함 
	...
	void display() {
		...
		JLabel l = new JLabel("Name" , new ImageIcon("3월30일/c_info/imgs/4.PNG") , 2);
