...
class InfoView implements ActionListener{
	...
	public InfoView() {
		...
		*/				
		...
		try {
			model = new InfoModelimpl(); // 부모 데이터 타입에 자식 객체 생성 가능 및 오버라이딩 된 자식객체의 함수를 사용
	...
	public void actionPerformed( ActionEvent ev ){
		Object evt = ev.getSource(); // 지금은 j버튼 형변환이 안됨
		...
		}else if(evt == tf_tel) { // tf_tel 에서 이벤트가 발생 했을 때
			selectBytel();
	...
	void selectBytel() {
		...
		//1. 사용자가 분명 tel칸에 번화번호를 치고 enter를 칠 것이다. / 이 값을 가져온다.
		...
		// 화면이 하는일은 사용가자 입력한 이 번호를 보낸다.
		//3. 2번객체의 각각의 값을 각각의 텍스트 필드에 지정
	...
	void addRecord() {
		...
		try {
			//1.add 버튼을 눌렀을 때 각 tf에서 값을 가져온다.
			...
			//하지만 이런식으로 하면 나중에 입력 할 값들이 너무 많아지면 , 헷갈리기 때문에 따로 클래스를 만든다.
			
			//2.각각의 값들을 infoVO의 값으로 지정
	...
	void showRecord() {
		...
		try {
			ArrayList<InfoVO> list =model.selectAll(); // model의 select 함수를 , 호출 
													 // select all 에 의해서 list에 각각의 row가 저장되어있고
													
			
			for(int i=0; i<list.size(); i++) {
				InfoVO vo = (InfoVO)list.get(i); // 그 리스트에 저장된 i번째 객체를 가져와서 다시 저장한뒤에
				ta.append(vo.toString()); // vo에 오버라이딩한 함수를 불러와서 textarea 에 추가.
			}
			
			
			
		} catch (SQLException e) {
			ta.setText("전체 보기 실패 : " + e.getMessage());
		}
	...
	void modifyRecord() {
		...
		try {
			...
			//2.각각의 값들을 infoVO의 값으로 지정 하고 model의 modify로 값을 넘김
